...
declare class Rekognition extends Service {
  ...
  /**
   * Detects Personal Protective Equipment (PPE) worn by people detected in an image. Amazon Rekognition can detect the following PPE.   Masks   Gloves   Helmets   You pass the input image as base64-encoded image bytes or as a reference to an image in an Amazon S3 bucket. The image must be either a PNG or JPG formatted file.   DetectProtectiveEquipment detects PPE worn by up to 10 persons detected in an image. For each person detected in the image the API returns an array of body parts (face, head, left-hand, right-hand). For each body part, an array of detected items of PPE is returned. The API returns the confidence it has in each detection (person, body part, PPE). It also returns a bounding box (BoundingBox) for each detected person and each detected item of PPE.  This is a stateless API operation. That is, the operation does not persist any data. This operation requires permissions to perform the rekognition:DetectProtectiveEquipment action. 
   */
...
declare namespace Rekognition {
  ...
  export type EquipmentType = "MASK"|"GLOVE"|"HELMET"|string;
